        ...
        controller: ['$scope', '$element', '$attrs', function($scope, $element, $attrs) {
                ...
                var options = parent.options,
                    ...
                    on = function(type, handler) {
                        ...
                        if(handler && jQuery.isFunction(handler)) {
                            $(window.document)
                                .on(browserEvent, handler)
                                .on(mobileEvent, handler);
                        } else {
                            $(window.document)
                                .off(browserEvent)
                                .off(mobileEvent);
            ...
            imageSelectAreas.prototype.init = function(object, customOptions) {
                ...
                if(this.options.allowSelect) {
                    ...
                    this.$trigger.mousedown($.proxy(this.newArea, this))
                        .on("touchstart", $.proxy(this.newArea, this));
                }
                if(this.options.allowNudge) {
                    $('html')
                        .keydown(function(e) { // move selection with arrow keys
                            var codes = {
                                    37: "l",
                                    38: "u",
                                    39: "r",
            ...
            imageSelectAreas.prototype.newArea = function(event) {
                var areaid = -1;
                this.blurAll();
                if(this.options.maxAreas && this.options.maxAreas <= this.areas()
                    .length) {
                    return areaid;
                }
